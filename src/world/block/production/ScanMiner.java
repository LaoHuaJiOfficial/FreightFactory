...
public class ScanMiner extends Block{
    ...
    protected void countOre(Tile tile, int rotation){
        ...
        for (int x = 0; x < area; x++) {
            for (int y = 0; y < area; y++) {
                Tile other;
                if (rotation == 0){
                    other = Vars.world.tile(tx + size + x, ty + y);
                }else if (rotation == 1){
                    other = Vars.world.tile(tx + x, ty + size + y);
                }else if (rotation == 2){
                    other = Vars.world.tile(tx - size + x, ty + y);
                }else if (rotation == 3){
                    other = Vars.world.tile(tx + x, ty - size + y);
    ...
    public class ScanMinerBuild extends Building{
        ...
        public void buildConfiguration(Table table){
            ItemSelection.buildTable(ScanMiner.this, table, content.items(), () -> sortItem, this::configure, selectionRows, selectionColumns);
        }
        ...
        public void drawSelect(){
            if(dominantItem != null){
                ...
                Draw.reset();
                Drawf.dashRect(Tmp.c1.set(Pal.accent), getRect(Tmp.r1, x, y, rotation));
            }
        }
        ...
        public void updateTile(){
            ...
            if(items.total() < itemCapacity && dominantItems > 0 && efficiency > 0){
                ...
                Lines.square(x + Mathf.sin(rotation), y + Mathf.cos(rotation), 15, 45);
