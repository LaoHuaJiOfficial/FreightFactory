...
public class HudFragmentF extends HudFragment {
    ...
    //todo @Remote(targets = Loc.both, forward = true, called = Loc.both)
    ...
    public void build(Group parent) {
        ...
        parent.fill(cont -> {
            ...
            cont.update(() -> {
                if (Core.input.keyTap(Binding.toggle_menus) && !ui.chatfrag.shown() && !Core.scene.hasDialog() && !Core.scene.hasField()) {
                    Core.settings.getBoolOnce("extra.ui-hidden", () -> {
            ...
            wavesMain.table(s -> {
                ...
                var rightStyle = new ImageButton.ImageButtonStyle() {{
        ...
        parent.fill(t -> {
            ...
            t.table(c -> {
                ...
                Events.run(EventType.Trigger.teamCoreDamage, () -> coreAttackTime[0] = notifDuration);
    ...
    private Table makeStatusTable() {
        ...
        table.clicked(() -> {
            if(state.rules.objectives.any()){
                ...
                extra.ui.showInfo(text.toString());
            }
        });*/
